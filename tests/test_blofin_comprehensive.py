#!/usr/bin/env python3
"""
Comprehensive BloFin test comparing native processor with CCXT processor.
...
class BloFinComprehensiveTest:
    def __init__(self):
        self.native = None
        self.ccxt = None
        self.test_symbol = "ADA-USDT"  # Native format
    ...
    async def setup(self):
        ...
        print(f"{Fore.CYAN}Setting up BloFin processors...")
        print(f"{Fore.CYAN}{'='*60}")
        
        # Initialize native processor
        self.native = BloFin()
        print(f"{Fore.GREEN}✓ Native BloFin processor initialized")
        
        # Initialize CCXT processor with BloFin credentials
        all_credentials = load_ccxt_credentials()
        blofin_cred = None
        for cred in all_credentials.ccxt_list:
            if cred.exchange_name.lower() == 'blofin':
                blofin_cred = cred
                break
        
        if not blofin_cred:
            raise ValueError("BloFin credentials not found in CCXT list")
            
        self.ccxt = CCXTProcessor(ccxt_credentials=blofin_cred)
        print(f"{Fore.GREEN}✓ CCXT BloFin processor initialized")
    ...
    async def test_position_lifecycle(self):
        ...
        # Test 3: Change margin mode (BloFin requires closing position)
        print(f"\n{Fore.YELLOW}Test 3: Changing margin mode to cross...")
        await self.native.reconcile_position(
            symbol=self.test_symbol,
            size=-self.initial_position_size,
            leverage=10,
        ...
        await self.compare_positions("After margin mode change")
...
async def main():
    """Run the comprehensive BloFin test."""
    tester = BloFinComprehensiveTest()
