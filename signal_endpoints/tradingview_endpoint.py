...
domain_name = input("Enter the domain name for the TradingView endpoint (e.g., tv-webhook.domain.com): ")
...
# Move the file to the required directory with sudo
...
async def tradingview_webhook(request: Request):
    ...
    # Log entry
    log_entry = f"{timestamp} {body.decode('utf-8').strip()}\n"
    
    # Store logs in a secure directory
    log_dir = Path("/var/log/tradingview_webhook")
