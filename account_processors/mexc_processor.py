...
from config.credentials import load_mexc_credentials, default_min_order_notional_for_exchange
...
class MEXC:
    def __init__(self):
        ...
        self.min_order_notional_usd = float(
            self.credentials.mexc.min_order_notional_usd
            if getattr(self.credentials.mexc, "min_order_notional_usd", 0) > 0
            else default_min_order_notional_for_exchange("mexc")
        )
    ...
    async def open_market_position(self, symbol: str, side: str, size: float, leverage: int, margin_mode: str, scale_lot_size: bool = True):
        ...
        try:
            ...
            if lots == 0:
                print(f"{self.log_prefix} Skipping {symbol} order: size below minimum/step.")
                return None
